...
# aes 함수에 color를 추가한다면 어떻게 될까?#잘못된 형태임.
ggplot(covid_kor, aes(x=ObservationDate)) + geom_line(aes(y=Confirmed, color="red")) + geom_line(aes(y=Current, color="black")) + geom_line(aes(y=Recovered, color="green")) + geom_line(aes(y=Deaths, color="blue")) + scale_x_date(date_labels = "%m-%d") + labs(title = "COVID-19 환자 수 변화 그래프", x="", y="환자 수", caption = "Source: https://www.kaggle.com/sudalairajkumar/novel-corona-virus-2019-dataset") 

# scale_color_manual 활용하여 범례 표시 수정#color에 임의의 이름을 붙혀주고 scale함수로 이름에따라서 범례와 색깔을 지정해줌.
...
# 4개로 나누어진 환자유형을 하나의 칼럼으로 합쳐줌. 시각화하기 더 쉬움.
# tidyr 패키지의 gather() 사용
# 원래 데이터에서 confiremd 부터 current까지를 하나의 칼럼으로 만들자
covid_kor2 <- gather(covid_kor, key="type", value="count", Confirmed:Current)
str(covid_kor2)


# line plot 동시에 작성
ggplot(covid_kor2, aes(x=ObservationDate, y=count, color=factor(type, levels=c("Confirmed", "Current", "Recovered", "Deaths"), labels=c("확진", "치료중","완치","사망")))) + geom_line() + scale_x_date(date_labels = "%m-%d") + labs(title = "COVID-19 환자 수 변화 그래프", x="", y="환자 수", caption = "Source: https://www.kaggle.com/sudalairajkumar/novel-corona-virus-2019-dataset", color="환자 분류")
```
<br/>
...
ggplot(covid_2s, aes(x=ObservationDate, y=count, color=factor(type, levels=c("Confirmed", "Current", "Recovered", "Deaths"), labels=c("확진", "치료중","완치","사망")))) + geom_line() + scale_x_date(date_labels = "%m-%d") + labs(title = "COVID-19 환자 수 변화 그래프", x="", y="환자 수", caption = "Source: https://www.kaggle.com/sudalairajkumar/novel-corona-virus-2019-dataset", color="환자 분류") + facet_wrap(~Country.Region) + theme_fivethirtyeight() +  theme(text = element_text(family = ""))
```
...
# customizing
p + labs(x="", y="확진자 수", title="국가 별 COVID-19 확진자 수", subtitle="2020년 3월 24일 기준", caption = "Source: https://www.kaggle.com/sudalairajkumar/novel-corona-virus-2019-dataset") + theme_fivethirtyeight() + theme(text = element_text(family = ""))
